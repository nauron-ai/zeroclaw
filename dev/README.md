...
- The CI image keeps Rust toolchain defaults from `rust:1.94-slim` and installs pinned toolchain `1.94.0` (no custom `CARGO_HOME`/`RUSTUP_HOME` overrides), preventing repeated toolchain bootstrapping on each run.
