...
INSTALLER_LOCAL="$(cd "$SCRIPT_DIR/.." >/dev/null 2>&1 && pwd || pwd)/labaclaw_install.sh"
BOOTSTRAP_LOCAL="$SCRIPT_DIR/bootstrap.sh"
REPO_URL="https://github.com/nauron-ai/labaclaw.git"

echo "[deprecated] scripts/install.sh -> ./labaclaw_install.sh" >&2
...
TEMP_DIR="$(mktemp -d -t labaclaw-install-XXXXXX)"
cleanup() {
  rm -rf "$TEMP_DIR"
}
trap cleanup EXIT
...
if [[ -x "$TEMP_DIR/labaclaw_install.sh" ]]; then
  exec "$TEMP_DIR/labaclaw_install.sh" "$@"
fi
...
echo "error: labaclaw_install.sh/bootstrap.sh was not found in the fetched revision." >&2
echo "Run the local bootstrap directly when possible:" >&2
echo "  ./labaclaw_install.sh --help" >&2
exit 1
