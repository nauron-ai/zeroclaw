...
#[path = "wizard_hybrid_memory.rs"]
mod wizard_hybrid_memory;
...
pub async fn run_wizard_with_migration(
    ...
    println!(
        "  {}",
        style("Welcome to LabaClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 11, "Channels (How You Talk to LabaClaw)");
    ...
    let mut config = Config {
        ...
        provider_timeout_secs: crate::config::schema::resolved_provider_timeout_secs(),
        observability: ObservabilityConfig::default(),
        autonomy: AutonomyConfig::default(),
        security: crate::config::SecurityConfig::default(),
        runtime: RuntimeConfig::default(),
        worker_plane: crate::config::WorkerPlaneConfig::default(),
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to LabaClaw)");
...
/// Use `labaclaw onboard` or `labaclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid|cortex-mem`.
/// Use `labaclaw onboard --interactive` for the full wizard.
...
async fn maybe_run_openclaw_migration(
    ...
    let should_run = if options.enabled {
        ...
        Confirm::with_theme(wizard_theme())
            .with_prompt(
                "  Merge OpenClaw data into this LabaClaw workspace now? (preserve existing data)",
            )
            .default(true)
            .interact()?
    } else {
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    if let Ok(custom_config_dir) = std::env::var("LABACLAW_CONFIG_DIR") {
    ...
    if let Ok(custom_workspace) = std::env::var("LABACLAW_WORKSPACE") {
    ...
    let config_dir = home.join(".labaclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (labaclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     labaclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  labaclaw gateway");
            println!("    3. Status:   labaclaw status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             labaclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              labaclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           labaclaw gateway");
                println!("    4. Status:            labaclaw status");
            }
        } else {
            ...
            println!("    2. Or edit:           ~/.labaclaw/config.toml");
            println!("    3. Chat:              labaclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           labaclaw gateway");
        }
    } else {
        println!("    1. Chat:     labaclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  labaclaw gateway");
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("LabaClaw works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if key.trim().is_empty() {
            print_bullet(
                "No token provided. LabaClaw will open the GitHub device login flow on first use.",
            );
        }
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("LabaClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how LabaClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("LabaClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("LabaClaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("LabaClaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::with_theme(wizard_theme())
        .with_prompt("  How should LabaClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how LabaClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
    ...
    let backend_kind = classify_memory_backend(backend);
    if matches!(
        backend_kind,
        MemoryBackendKind::SqliteQdrantHybrid | MemoryBackendKind::PostgresQdrantHybrid
    ) {
        wizard_hybrid_memory::configure_hybrid_qdrant_memory(&mut config, backend_kind)?;
    }
...
fn setup_identity_backend() -> Result<IdentityConfig> {
    print_bullet("Choose the identity format LabaClaw should scaffold for this workspace.");
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to LabaClaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to LabaClaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to LabaClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to LabaClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("LabaClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::with_theme(wizard_theme())
                        ...
                        .default("~/.labaclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::with_theme(wizard_theme())
                    ...
                    .default("labaclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via LABACLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to LabaClaw from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("LabaClaw will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("LabaClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if canonical_provider == "copilot" {
            ...
            println!("       {}", style("labaclaw agent -m \"Hello!\"").yellow());
            println!(
                "       {}",
            ...
            println!(
                "       {}",
                style("labaclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: labaclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("labaclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("labaclaw agent -m \"Hello, LabaClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("labaclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("labaclaw status").yellow());
...
mod tests {
    ...
    async fn run_quick_setup_with_clean_env(
        ...
        let _workspace_env = EnvVarGuard::unset("LABACLAW_WORKSPACE");
        let _config_env = EnvVarGuard::unset("LABACLAW_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        let tmp = TempDir::new().unwrap();
        let labaclaw_dir = tmp.path().join(".labaclaw");
        let config_path = labaclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&labaclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        ...
        let expected_config_path = workspace_root.join(".labaclaw").join("config.toml");
        ...
        .await
        .expect("quick setup should honor LABACLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** LabaClaw"),
            "should default agent name to LabaClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("LabaClaw-v2"));
    }
    ...
    fn backend_key_from_choice_maps_supported_backends() {
        ...
        assert_eq!(backend_key_from_choice(2), "postgres_qdrant_hybrid");
        assert_eq!(backend_key_from_choice(3), "lucid");
        assert_eq!(backend_key_from_choice(4), "cortex-mem");
        assert_eq!(backend_key_from_choice(5), "markdown");
    ...
    fn memory_config_defaults_for_hybrid_enable_sqlite_hygiene() {
        ...
        assert_eq!(config.purge_after_days, 30);
        assert_eq!(config.embedding_cache_size, 10000);
        assert_eq!(config.qdrant.collection, "labaclaw_memories");
    }
