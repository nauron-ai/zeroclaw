...
Examples:
  ...
  labaclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  labaclaw channel bind-telegram zeroclaw_user
  labaclaw channel bind-telegram 123456789")]
    BindTelegram {
...
Examples:
  labaclaw channel send --channel whatsapp_web --to +15551234567 --message \"done\"
  echo \"hello\" | labaclaw channel send --channel telegram --to 123456 --message -
")]
    Send {
...
pub enum MigrateCommands {
    /// Import OpenClaw data into this LabaClaw workspace (memory, config, agents)
    Openclaw {
...
Examples:
  ...
  labaclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  labaclaw cron add-every 60000 'Ping heartbeat'     # every minute
  labaclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  labaclaw cron once 2h 'Follow up on deployment'
  labaclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  labaclaw cron update <task-id> --expression '0 8 * * *'
  labaclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  labaclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  labaclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  labaclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash LabaClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash LabaClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  labaclaw peripheral flash
  labaclaw peripheral flash --port /dev/cu.usbmodem12345
  labaclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash LabaClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
