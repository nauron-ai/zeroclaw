...
pub async fn run_tool_call_loop(
    ...
    let heartbeat_config = SAFETY_HEARTBEAT_CONFIG
        ...
        .ok()
        .flatten();
    let dedup_exempt_tools = TOOL_LOOP_DEDUP_EXEMPT
        .try_with(Clone::clone)
        .unwrap_or_default();
    ...
    for iteration in 0..max_iterations {
        ...
        for (idx, call) in tool_calls.iter().enumerate() {
            ...
            let dedup_exempt = dedup_exempt_tools.iter().any(|name| name == &tool_name);
...
pub async fn run(
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload agent-generated Arduino sketch. Use when: user asks for 'make a heart', 'blink pattern', or custom LED behavior on Arduino. You write the full .ino code; LabaClaw compiles and uploads it. Pin 13 = built-in LED on Uno.",
        ));
        tool_descs.push((
            "hardware_memory_map",
    ...
    if let Some(msg) = message {
        ...
        println!("🦀 LabaClaw Interactive Mode");
...
pub async fn process_message_with_session(
    ...
    if config.peripherals.enabled && !config.peripherals.boards.is_empty() {
        ...
        tool_descs.push((
            "arduino_upload",
            "Upload Arduino sketch. Use for 'make a heart', custom patterns. You write full .ino code; LabaClaw uploads it.",
        ));
        tool_descs.push((
            "hardware_memory_map",
