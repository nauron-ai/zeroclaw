...
//! LabaClaw to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "labaclaw.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "labaclaw.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["labaclaw", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["labaclaw", "restart"];
    ...
    match channel_name {
        ...
        "whatsapp" => Some(
            "When responding on WhatsApp:\n\
             ...
             - The path MUST be an absolute filesystem path to a local file (e.g. [IMAGE:/home/nicolas/.labaclaw/workspace/images/chart.png]).\n\
             - Keep normal text outside markers and never wrap markers in code fences.\n\
...
async fn load_runtime_defaults_from_config_file(
    ...
    if let Some(labaclaw_dir) = path.parent() {
        let store = crate::security::SecretStore::new(labaclaw_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `labaclaw models refresh --provider {}`.",
...
pub fn build_system_prompt_with_mode(
    ...
    if prompt.is_empty() {
        "You are LabaClaw, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `labaclaw onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `labaclaw daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `labaclaw service stop && labaclaw service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.labaclaw.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.labaclaw.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.labaclaw.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.labaclaw.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/labaclaw");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: labaclaw channel start");
            println!("To check health:    labaclaw channel doctor");
            println!("To configure:      labaclaw onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `labaclaw onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.labaclaw/config.toml directly");
        }
...
fn collect_configured_channels(
    ...
    if let Some(ref mx) = config.channels_config.matrix {
        channels.push(ConfiguredChannel {
            display_name: "Matrix",
            channel: Arc::new(
                MatrixChannel::new_with_session_hint_and_labaclaw_dir(
                    mx.homeserver.clone(),
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() && init_failures.is_empty() {
        println!("No real-time channels configured. Run `labaclaw onboard` first.");
        return Ok(());
    }

    println!("🩺 LabaClaw Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `labaclaw gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if configured_channels.is_empty() && init_failures.is_empty() {
        println!("No channels configured. Run `labaclaw onboard` to set up channels.");
        return Ok(());
    }
    ...
    println!("🦀 LabaClaw Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from LabaClaw 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
BTC is currently around $65,000 based on latest tool output."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "labaclaw.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "labaclaw.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["labaclaw", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["labaclaw", "restart"]);
    }
