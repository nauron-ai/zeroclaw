# Network Deployment — LabaClaw on Raspberry Pi and Local Network

This document covers deploying LabaClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | LabaClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | LabaClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — LabaClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. LabaClaw on Raspberry Pi
...
- Telegram channel works: LabaClaw polls Telegram API (outbound)
...
```bash
labaclaw daemon --host 0.0.0.0 --port 42617
```
...
- LabaClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `labaclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `labaclaw daemon` service).
...
- [ ] Run `labaclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
LabaClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `labaclaw` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo labaclaw service install
```

This creates:
- Init script: `/etc/init.d/labaclaw`
- Config directory: `/etc/labaclaw/`
- Log directory: `/var/log/labaclaw/`
...
# Check status
sudo rc-service labaclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service labaclaw start` | Start the daemon |
| `sudo rc-service labaclaw stop` | Stop the daemon |
| `sudo rc-service labaclaw status` | Check service status |
| `sudo rc-service labaclaw restart` | Restart the daemon |
| `sudo labaclaw service status` | LabaClaw status wrapper (uses `/etc/labaclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/labaclaw/access.log` |
| Errors/stderr | `/var/log/labaclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/labaclaw/error.log
```
...
sudo rc-service labaclaw stop
sudo rc-update del labaclaw default

# Remove init script
sudo labaclaw service uninstall
```
