...
Issue: [#2263](https://github.com/nauron-ai/labaclaw/issues/2263)
...
```bash
scripts/ci/m4_5_rfi_baseline.sh /tmp/labaclaw-m4rfi-target
```
...
1. `crates/labaclaw-types`
- shared DTOs, enums, IDs, lightweight cross-domain traits
- no provider/channel/network dependencies

1. `crates/labaclaw-core`
- config structs + validation, provider trait contracts, routing primitives, policy helpers
- depends on `labaclaw-types`

1. `crates/labaclaw-memory`
- memory traits/backends + hygiene/snapshot plumbing
- depends on `labaclaw-types`, `labaclaw-core` contracts only where required

1. `crates/labaclaw-channels`
- channel adapters + inbound normalization
- depends on `labaclaw-types`, `labaclaw-core`, `labaclaw-memory`

1. `crates/labaclaw-api`
- gateway/webhook/http orchestration
- depends on `labaclaw-types`, `labaclaw-core`, `labaclaw-memory`, `labaclaw-channels`

1. `crates/labaclaw-bin` (or keep root binary package name stable)
...
- keep provider-specific SDK deps out of `labaclaw-types`
...
- add package-focused lanes for extracted crates (`-p labaclaw-types`, `-p labaclaw-core`, etc.)
...
- extract low-churn shared types into `labaclaw-types`
- add re-export shim layer to preserve existing import paths

### PR-3 (S)

- extract config/provider contracts into `labaclaw-core`
