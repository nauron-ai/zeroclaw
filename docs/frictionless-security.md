...
[INFO] ✓ Audit logging enabled (~/.config/labaclaw/audit.log)
...
```bash
# Check what's active
$ labaclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/labaclaw/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ labaclaw config set security.sandbox.enabled false

# Enable specific backend
$ labaclaw config set security.sandbox.backend firejail

# Adjust limits
$ labaclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ labaclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `labaclaw security --status`
