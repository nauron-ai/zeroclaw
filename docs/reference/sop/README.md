...
- CLI `labaclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.labaclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.labaclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   labaclaw sop list
   labaclaw sop validate
   labaclaw sop show deploy-prod
   ```
