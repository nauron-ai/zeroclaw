# LabaClaw Security Improvement Roadmap
...
LabaClaw already has **excellent application-layer security**:
...
## Comparison: LabaClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | LabaClaw Now | LabaClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`labaclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/labaclaw/audit.log"
sign_events = true
max_size_mb = 100
...
labaclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
labaclaw audit --user @alice --since 24h
labaclaw audit --risk high --violations-only
labaclaw audit --verify-signatures

# Sandbox test
labaclaw sandbox --test
# → Testing isolation...
...
**LabaClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, LabaClaw becomes:
...
**Value**: Transforms LabaClaw from "safe for testing" to "safe for production"
