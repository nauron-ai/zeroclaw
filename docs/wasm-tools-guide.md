...
(skills) in LabaClaw. WASM tools let you extend the agent with custom capabilities
written in any language that compiles to WebAssembly — without modifying LabaClaw's
core source code.
...
7. [How LabaClaw Loads and Uses the Tool](#7-how-labaclaw-loads-and-uses-the-tool)
...
└───────────────────────┬─────────────────────────────────────┘
                        │  WASI stdio protocol
┌───────────────────────▼─────────────────────────────────────┐
│  LabaClaw WASM engine (wasmtime + WASI)                     │
│                                                             │
...
| Requirement | Purpose |
|---|---|
| LabaClaw built with `--features wasm-tools` | Enables the WASM runtime |
| `wasmtime` CLI | Local testing (`labaclaw skill test`) |
...
```bash
labaclaw skill new <name> --template <typescript|rust|go|python>
```

Example:

```bash
labaclaw skill new weather_lookup --template rust
```
...
**Input** (written to the tool's stdin by LabaClaw):
...
`parameters`. LabaClaw passes the LLM-provided argument object verbatim.

**Output** (read from the tool's stdout by LabaClaw):
...
LabaClaw the tool's name, description, and the JSON Schema for its parameters.
...
| `homepage` | no | Optional URL shown in `labaclaw skill list` |
...
componentize-py -d wit/ -w labaclaw-skill componentize app -o tool.wasm
```
...
| Python | `componentize-py -d wit/ -w labaclaw-skill componentize app -o tool.wasm` | `tool.wasm` |
...
Before installing, test the tool directly without starting the full LabaClaw agent:

```bash
labaclaw skill test . --args '{"city":"Hanoi","units":"metric"}'
```

You can also test an installed skill by name:

```bash
labaclaw skill test weather_lookup --args '{"city":"Tokyo"}'
```

Or test a specific tool inside a multi-tool skill:

```bash
labaclaw skill test . --tool my_tool_name --args '{"city":"Paris"}'
```
...
```bash
labaclaw skill install ./weather_lookup
```

This copies your skill directory into `<workspace>/skills/weather_lookup/`.
LabaClaw will auto-discover it on next startup.

### 6.2 Install from a git repository

```bash
labaclaw skill install https://github.com/yourname/weather_lookup.git
```

LabaClaw clones the repository into the skills directory and scans for WASM tools.

### 6.3 Install from ZeroMarket registry

```bash
# Format: namespace/package-name
labaclaw skill install acme/weather-lookup

# With a specific version
labaclaw skill install acme/weather-lookup@0.2.1
```

LabaClaw fetches the package index from the configured registry URL, then downloads
...
## 7. How LabaClaw Loads and Uses the Tool

### 7.1 Startup discovery

Every time the LabaClaw agent starts, it scans the `skills/` directory and loads
...
Agent: [internally, LLM selects tool "weather_lookup" with args {"city":"Hanoi"}]
       ...
       LabaClaw calls weather_lookup WASM tool:
         stdin  → {"city":"Hanoi"}
...
If a tool fails (non-zero exit, invalid JSON, timeout, fuel exhaustion), LabaClaw
...
**Installed layout** (created by `labaclaw skill install`):
...
Add this section to your `labaclaw.toml` to tune WASM tool behavior:
...
# Registry URL used by `labaclaw skill install namespace/package`
...
**Tool not appearing in `labaclaw skill list`**
