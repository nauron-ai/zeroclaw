# Hardware Peripherals Design — LabaClaw

LabaClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** LabaClaw acts as a hardware-aware AI agent that:
...
**Mental model:** LabaClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
LabaClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  LabaClaw on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. LabaClaw fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
LabaClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  LabaClaw on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. LabaClaw identifies connected hardware (VID/PID, architecture)
...
2. LabaClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. LabaClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| LabaClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs LabaClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

LabaClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
board = "esp32"
transport = "wifi"
# Edge-Native: LabaClaw runs on ESP32
```
...
1. **Startup:** LabaClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between LabaClaw and peripherals:
...
- **labaclaw-firmware** or **labaclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; LabaClaw connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`labaclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `labaclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `labaclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] LabaClaw on Raspberry Pi (native GPIO via rppal)
...
- [x] `labaclaw-esp32` firmware crate (`firmware/labaclaw-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] LabaClaw *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/labaclaw-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full LabaClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). LabaClaw runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. LabaClaw gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: LabaClaw from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey LabaClaw, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
