# Adding Boards and Tools — LabaClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to LabaClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.labaclaw/config.toml)
labaclaw peripheral add nucleo-f401re /dev/ttyACM0
labaclaw peripheral add arduino-uno /dev/cu.usbmodem12345
labaclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, LabaClaw can index PDF files:
...
2. **Add to config** — `labaclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `labaclaw peripheral list` | List configured boards |
| `labaclaw peripheral add <board> <path>` | Add board (writes config) |
| `labaclaw peripheral flash` | Flash Arduino firmware |
| `labaclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `labaclaw hardware discover` | List USB devices |
| `labaclaw hardware info` | Chip info via probe-rs |
